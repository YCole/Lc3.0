...
import com.gome.launcher.widget.WidgetCell;
...
public class WidgetPreviewLoader {
    ...
    @Thunk Bitmap generatePreview(Launcher launcher, Object info, Bitmap recycle,
        ...
        if (info instanceof LauncherAppWidgetProviderInfo) {
            return generateWidgetPreview(launcher, (LauncherAppWidgetProviderInfo) info,
                    previewWidth, previewHeight, recycle, null);//mod by rongwenzhao 2017-7-7
        } else {
    ...
    public Bitmap generateWidgetPreview(Launcher launcher, LauncherAppWidgetProviderInfo info,
        ...
        if (previewWidth > profile.mPresetPreviewWidgetWidth) {
            scale =   profile.mPresetPreviewWidgetWidth/ (float) (previewWidth);
        }

        //add by rongwenzhao begin
        if(previewHeight > profile.mPresetPreviewWidgetHight) {
            scale = Math.min( profile.mPresetPreviewWidgetHight/(float) previewHeight , scale);
        }
        //add by rongwenzhao end

        DLog.d(TAG,"generateWidgetPreview scale = " + scale);
        if (scale != 1f) {
        ...
        int y = (preview.getHeight() - previewHeight) / 2; //add by rongwenzhao 2017-6-27
        if (widgetPreviewExists) {
            ///M. ALPS002270464. Modify the area we need,
            ///from (0,0) is better.Just like L1.
            // now modify the area from (x,y)   add by rongwenzhao
            //drawable.setBounds(0, 0, previewWidth, previewHeight); //delete by rongwenzhao 2017-6-27
            drawable.setBounds(x, y, x + previewWidth, y + previewHeight); //add by rongwenzhao 2017-6-27
            drawable.draw(c);
        } else {
            ...
            for (int i = 0; i < spanX; i++, tx += tileW) {
                float ty = y;//mod by rongwenzhao orign code:   ty = 0; 2017-6-27
            ...
            try {
                ...
                if (icon != null) {
                    ...
                    int yoffset = (int) ((tileH - appIconSize * iconScale) / 2) + y;//mod by rongwenzhao orign code is : int yoffset = (int) ((tileH - appIconSize * iconScale) / 2); 2017-6-27
    ...
    private Bitmap generateShortcutPreview(
        ...
        //now draw the final icon in the center of the view     add by rongwenzhao 2017-6-27
        // TODO: use top right for RTL
        icon.setAlpha(255);
        icon.setColorFilter(null);
        //icon.setBounds(0, 0, appIconSize, appIconSize); //delete by rongwenzhao
       int  previewWidth = icon.getIntrinsicWidth();
        int  previewHeight = icon.getIntrinsicHeight();
        float scale =1f;
        DeviceProfile profile = launcher.getDeviceProfile();
        if (previewWidth > profile.mPresetPreviewWidgetWidth) {
            scale =   profile.mPresetPreviewWidgetWidth/ (float) (previewWidth);
        ...
        icon.setBounds(x, y, x + previewWidth, y + previewHeight);//add by rongwenzhao 2017-6-27
        icon.draw(c);
