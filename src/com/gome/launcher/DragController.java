...
import com.gome.launcher.accessibility.DragViewStateAnnouncer;
...
public class DragController {
    ...
    public static final int RESCROLL_DELAY = PagedView.PAGE_SNAP_ANIMATION_DURATION + 450;
    //update by huanghaihao for snap error on drop on 2017/8/3 start
    ...
    /**
     * Callback launcher when ShortCut  is delete
     */
    public interface ShortCutDeleteListener {
        void completeDrop(ItemInfo itemInfo);
    }

    public interface ShortCutDeletableListener {
        boolean canDrop(ItemInfo itemInfo);
    }
    ...
    // Added by wjq in 2017-11-14 for fix bug PRODUCTION-5318  start
    public void cancelDragByPkgName(String pkgName) {
        ...
        if (mDragObject.dragInfo instanceof ShortcutInfo) {
            shortcutInfo = (ShortcutInfo) mDragObject.dragInfo;

        }
        String shortcutInfoPkgName = "";
        if (shortcutInfo != null && shortcutInfo.getPkgName() != null) {
            shortcutInfoPkgName = shortcutInfo.getPkgName();
        }
        if (shortcutInfoPkgName.equals(pkgName)) {
            if (mDragging) {
                if (mLastDropTarget != null) {
                    mLastDropTarget.onDragExit(mDragObject);
                }
    ...
    // Added by wjq in 2017-11-14 for fix bug PRODUCTION-5318  end
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mDragging) {
                    ...
                    if (!DeleteDropTarget.supportsDrop(mLauncher, mDragObject.dragInfo)) {
                        vec = null;
                    }
                    if (vec != null) {
    ...
    private void drop(float x, float y) {
        ...
        if (dropTarget instanceof  DeleteDropTarget) {
            ((DeleteDropTarget)dropTarget).setShortCutDeleteListener(mShortCutDeleteListener);
            ((DeleteDropTarget)dropTarget).setShortCutDeletableListener(mShortCutDeletableListener);
        }
        mDragObject.x = coordinates[0];
        mDragObject.y = coordinates[1];
        if (LauncherLog.DEBUG_DRAG) {
