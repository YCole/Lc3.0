...
import com.gome.launcher.util.LauncherEdgeEffect;
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    //update by huanghaihao for snap error on drop on 2017/8/3 start
    protected static final int PAGE_SNAP_ANIMATION_DURATION = 300;
    protected static final int PAGE_SNAP_ANIMATION_DURATION_MIN = 100;
    protected static final int SLOW_PAGE_SNAP_ANIMATION_DURATION = 500;
    //update by huanghaihao for snap error on drop on 2017/8/3 end
    ...
    public boolean mWasInOverscroll = false;
	...
	//linhai set isResetPageSpacing true when draging on  start 2017/11/15
    private boolean isResetPageSpacing;
	//linhai set isResetPageSpacing true when draging on   start 2017/11/15
    // Convenience/caching
    ...
    //add linhai Relating to the scroll and overscroll effects start 2017/6/19
    protected static float CAMERA_DISTANCE = 6500;
    protected static final float TRANSITION_SCALE_FACTOR = 0.74f;
    protected static final float TRANSITION_SCREEN_ROTATION = 60.0f;
    protected int mCameraDistance;
    protected boolean mScrollTransformsSet;
    ...
    //Feature: circular sliding
    protected boolean mIsSupportCircular = false;
    //Feature: rebound effect
    protected boolean mHasReboundEffect = false;
    // @}
     ...
     */
    //-------------------------------start--------------///
//    public boolean mIsFolderOpened = false;
    //-------------------------------end--------------///
    ...
    protected void updateCurrentPageScroll() {
        ...
        int newX = 0;
        //add linhai scroll mScroller is not finish when update currentPage  but mCurrentPage is changed start 2017/12/6
        int currentPage = mScroller.isFinished()?  mCurrentPage:getNextPage();
        //add linhai scroll mScroller is not finish when update currentPage  but mCurrentPage is changed end 2017/12/6
    ...
    private void forceFinishScroller() {
        // We need to clean up the next page here to avoid computeScrollHelper from
        // updating current page on the pass.
		//linhai not set forceFinished when on resetPageSpacing start 2017/11/15
        if (!isResetPageSpacing)
        {
        ...
        isResetPageSpacing = false;
		//linhai end not set forceFinished when on resetPageSpacing start 2017/11/15
    }
...
//        }
        ...
        invalidate();
        //linhai add for TransitionEffect 2017/6/19
//        postDelayed(runnable, PAGE_SNAP_ANIMATION_DURATION + 20);
        //linhai end 2017/6/19
    }
...
//        @Override
//        public void run() {
//            TransitionEffect.setIsEnterToOverviewMode(false);
//            postInvalidate();
//        }
//    };
    ...
    public void scrollTo(int x, int y) {
        ...
        if (stopScrolling() && x <= getScrollForPage(1)) {
            forceFinishScroller();
            mTouchState = TOUCH_STATE_REST;
            x= getScrollForPage(1);
            mOverScrollX = x;
            super.scrollTo(x,y);
            snapToPage(1, PAGE_SNAP_ANIMATION_DURATION_MIN);
        }
        // mod for folder icon by liuqiushou 20170620 @{
        else if (mIsSupportCircular || (x >= 0 && x <= mMaxScrollX) || mHasReboundEffect) {
            mOverScrollX = x;
            super.scrollTo(x, y);
        } else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = startIndex; i != endIndex; i += delta) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                //delete by liuning for update custompage pageSpacing on 2017/7/28 start
...
//                }
                //delete by liuning for update custompage pageSpacing on 2017/7/28 end
    ...
    /**
     * added by liuning for resetPageSpacing when draging on 2017/7/28
     * @param pageSpacing
     */
    public void resetPageSpacing(int pageSpacing) {
        mPageSpacing = pageSpacing;
        mFirstLayout = true;
        isResetPageSpacing = true;
        requestLayout();
    }


    /**
     * linhai add 获得 WorkspacePage空隙值
     * @return
     */
    ...
    /**
     *   Called when the center screen changes during scrolling.start 2017/6/19
     */
    protected void screenScrolled(int screenCenter) {
        //linhai add screenScrolled滑动 start 2017/6/19
        Log.d(TAG_TRANSITIONEFFECT, "pagedView screenScrolled :" + screenCenter);
        boolean isInOverscroll = mOverScrollX < 0 || mOverScrollX > mMaxScrollX;
        // Apply transition effect and adjacent screen fade if enabled
        int getCustomPageViewWidth = 0;
        if ((mTransitionEffect != null && mUseTransitionEffect) || mScrollTransformsSet) {
            for (int i = 0; i < getChildCount(); i++) {
                View v = getPageAt(i);
                if (v != null) {
                    float scrollProgress = getScrollProgress(screenCenter, v, i);
                    //deleted by jubingcheng for JumpEffect-fix start on 2017/6/28
//                    //  Fade first to allow transition effects to override alpha
//                    if (mTransitionEffect.mFadeInAdjacentScreens && !isInOverscroll) {
//                        float alpha = 1 - Math.abs(scrollProgress);
//                        setChildAlpha(v, alpha);
//                    }
                    //deleted by jubingcheng for JumpEffect-fix end on 2017/6/28
                    if (mTransitionEffect != null && mUseTransitionEffect && !isInOverscroll ) {
                        ...
                        if (this instanceof Workspace)
                        {
                            ...
                            //Added by linhai in 2017-9-25 for 切到负一屏的效果请改为平移
                            if (workspace.hasCustomContent()&&!workspace.isInOverviewMode())
                            {
                                if (getScrollX()>getCustomPageViewWidth+mPageSpacing/2)
                                {
                                    mTransitionEffect.screenScrolled(v, i, scrollProgress, mCurrentPage);
                                }else
                                {
    ...
    /**
     * linhai get current visibile pages update 2017/619
     * eg:  scrooling: [0,1] , stable[0,0]
     */
    protected void getVisiblePages(int[] range) {
        ...
        range[0] = -1;
        range[1] = -1;
        //add linhai 获得当页数 add 2017/6/19
        if (pageCount > 0) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    //added by liuning for can't scroll to custompage when in app manage mode on 2017/11/6 start
//                    if (stopScrolling() && deltaX < 0 &&  x <= mDownMotionX) {
//                        mTouchState = TOUCH_STATE_REST;
//                        snapToPage(1, PAGE_SNAP_ANIMATION_DURATION_MIN);
//                        return true;
//                    }
                    //added by liuning for can't scroll to custompage when in app manage mode on 2017/11/6 end
  ...
  public   int getPageNearestToCenterOfScreen() {
    ...
    /**
     * linhai
     * add 负一屏已经移除，view没有继续刷新问题
     */
    public   int getPageNearestToCenterOfScreenNoCustomPage() {
        int minDistanceFromScreenCenter = Integer.MAX_VALUE;
        int minDistanceFromScreenCenterIndex = -1;
        int screenCenter = getViewportOffsetX() + getScrollX() + (getViewportWidth() / 2);
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; ++i) {
            ...
            int childWidth = layout.getMeasuredWidth();
            int halfChildWidth = (childWidth / 2);
            int childCenter = getViewportOffsetX() + (getChildOffset(i) >(i+1)*getViewportWidth() ?  getChildOffset(i)-getViewportWidth() : getChildOffset(i)) + halfChildWidth;
            int distanceFromScreenCenter = Math.abs(childCenter - screenCenter);
            if (distanceFromScreenCenter < minDistanceFromScreenCenter) {
                minDistanceFromScreenCenter = distanceFromScreenCenter;
                minDistanceFromScreenCenterIndex = i;
...
//        }
        ...
        //linhai 清除当前获取焦点view焦点状态 add 2017/6/16
    ...
    public void scrollRight() {
        ...
        //全屏禁止滑动
        if (!isFullScreen && getNextPage() < getChildCount() - 1) snapToPage(getNextPage() + 1);
    }
    ...
    /**
     * If you are already in the OverView mode, the CellLayout will float up at this time by CellLayout.
     */
    ...
    /**
     * linhai 设置滑动时子view状态 add 2017/6/19
     * @param v
     */
    protected void setChildState(View v) {
    ...
    /**
     * linhai 设置滑动时子view Alpha值 add 2017/6/19
     *
     */
    protected void setChildAlpha(View child, float alpha) {
        ...
        if (child instanceof CellLayout) {
            //updated by huanghaihao in 2017-6-26 for screen setting start
            ((CellLayout) child).setShortcutAndWidgetAlpha(alpha);
            //updated by huanghaihao in 2017-6-26 for screen setting end
            ((CellLayout) child).getShortcutsAndWidgets().setVisibility(View.VISIBLE);
            if (this instanceof Workspace) {
                Workspace workspace = (Workspace) this;
                if (workspace.workspaceInModalState()) {
                    ((CellLayout) child).setBackgroundAlpha(Workspace.MAX_CELLLAYOUT_BACKGROUND_ALPHA * alpha);
                } else {
                    ((CellLayout) child).setBackgroundAlpha(0);
                }
            }
        }
    }
    ...
    /**
     * linhai 设置TransitionEffect实例 add 2017/6/19
     */
    ...
    private static final String TAG_TRANSITIONEFFECT = "transiton_effect";

    /**
     * linhai create 动画特效抽象类 2017/06/19
     */

    public static abstract class TransitionEffect {
        //平移
        public static final String TRANSITION_EFFECT_SLIDE = "none";
        //跃动
        public static final String TRANSITION_EFFECT_JUMP = "zoom-in";
        //淡入淡出
        public static final String TRANSITION_EFFECT_FADE = "fade_in_fade_out";
        //方盒（内)
        public static final String TRANSITION_EFFECT_SQUARE_BOX_IN = "cube-in";
        //方盒（外)
        public static final String TRANSITION_EFFECT_SQUARE_BOX_OUT = "cube-out";
        //叠加
        public static final String TRANSITION_EFFECT_SUPERPOSITION = "stack";
        //折叠
        public static final String TRANSITION_EFFECT_FOLD = "accordion";
        //翻转
        public static final String TRANSITION_EFFECT_TURN = "rotation";
...
//        public boolean mFadeInAdjacentScreens;
        ...
        private static boolean isEnterToOverviewMode = false;

        public static boolean isEnterToOverviewMode() {
            return isEnterToOverviewMode;
        }

        public static void setIsEnterToOverviewMode(boolean isEnterToOverviewMode) {
            TransitionEffect.isEnterToOverviewMode = isEnterToOverviewMode;
        }
        ...
        //linhai add 实现滑动特效的方法类 2017/06/19
        public void screenScrolled(View v, int i, float scrollProgress, int currentPage) {
            //linhai update 设置“平移”之外的切页特效后，进入预览模式时会显示其它页面问题修改 2017/06/19
            if (isEnterToOverviewMode()) {
                Log.d(TAG_TRANSITIONEFFECT, "screenScrolled isEnterToOverviewMode()");
                if (i != currentPage) {
                    v.setAlpha(0);
                }
                return;
            ...
            v.setAlpha(1);
            //end linhai 2017/06/19
            Log.d(TAG_TRANSITIONEFFECT, "screenScrolled");
        ...
        public static void setFromString(PagedView pagedView, String effect) {
            Log.e(TAG_TRANSITIONEFFECT, "setFromString :" + effect);
            if (effect.equals(TransitionEffect.TRANSITION_EFFECT_SLIDE)) {
                pagedView.setTransitionEffect(null);
            } else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_JUMP)) {
                pagedView.setTransitionEffect(new Zoom(pagedView, true));
            } else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_SQUARE_BOX_IN)) {
                pagedView.setTransitionEffect(new Cube(pagedView, true));
            } else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_SQUARE_BOX_OUT)) {
                pagedView.setTransitionEffect(new Cube(pagedView, false));
            }  else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_SUPERPOSITION)) {
                pagedView.setTransitionEffect(new Stack(pagedView));
            } else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_FOLD)) {
                pagedView.setTransitionEffect(new Accordion(pagedView));
            } else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_TURN)) {
                pagedView.setTransitionEffect(new Rotation(pagedView));
            } else if (effect.equals(TransitionEffect.TRANSITION_EFFECT_FADE)) {
                pagedView.setTransitionEffect(new Fade(pagedView));
            }
        }

        //linhai 创建各个动画子类 add 2017/6/19
        ...
        public static class Zoom extends TransitionEffect {
            ...
            public Zoom(PagedView pagedView, boolean in) {
                super(pagedView, TRANSITION_EFFECT_JUMP);
                //deleted by jubingcheng for JumpEffect-fix start on 2017/6/28
//                mFadeInAdjacentScreens = true;
            ...
            //added by jubingcheng for JumpEffect-fix start on 2017/6/28
            private DecelerateInterpolator mLeftScreenAlphaInterpolator = new DecelerateInterpolator(4);
            protected AccelerateInterpolator mAlphaInterpolator = new AccelerateInterpolator(0.9f);
            //added by jubingcheng for JumpEffect-fix end on 2017/6/28
            ...
            public void onScreenScrolled(View v, int i, float scrollProgress) {
                ...
                if (v instanceof CellLayout && mPagedView instanceof Workspace) {
                    ((CellLayout) v).setShortcutAndWidgetAlpha(alpha);
                    Workspace workspace = (Workspace) mPagedView;
                    if (workspace.workspaceInModalState()) {
                        ((CellLayout) v).setBackgroundAlpha(Workspace.MAX_CELLLAYOUT_BACKGROUND_ALPHA * alpha);
                    } else {
                        ((CellLayout) v).setBackgroundAlpha(0);
                    }
                } else {
                    v.setAlpha(alpha);
                }
                ...
                float scale = 1.0f + (mIn ? -0.25f : 0.1f) * Math.abs(scrollProgress);
        ...
        public static class Cube extends TransitionEffect {
            ...
            public Cube(PagedView pagedView, boolean in) {
                super(pagedView, in ? TRANSITION_EFFECT_SQUARE_BOX_IN : TRANSITION_EFFECT_SQUARE_BOX_OUT);
                //mFadeInAdjacentScreens=true;
                mIn = in;
            }
            ...
            public void onScreenScrolled(View v, int i, float scrollProgress) {
                ...
                int width = 0;
                int space = mPagedView.getPageSpacing();
                if (mIn) {
                    v.setCameraDistance(mPagedView.mDensity * 3 * PagedView.CAMERA_DISTANCE);
                    width = v.getMeasuredWidth() - space;
                    v.setPivotY(v.getMeasuredHeight() * 0.5f);
                } else {
                    width = v.getMeasuredWidth() + space;
                    v.setPivotY(v.getMeasuredHeight() * 0.5f);
                ...
                v.setPivotX(scrollProgress < 0 ? 0 : width);
//                v.setPivotY(v.getMeasuredHeight() * 0.5f);
        ...
        public static class Stack extends TransitionEffect {
            ...
            public Stack(PagedView pagedView) {
                super(pagedView, TRANSITION_EFFECT_SUPERPOSITION);
            }
            ...
            public void onScreenScrolled(View v, int i, float scrollProgress) {
                ...
                int space = mPagedView.getPageSpacing();
                int width = v.getMeasuredWidth() + space;
                ...
                if (alpha == 0) {
                    v.setTranslationX(v.getMeasuredWidth() * -1.0f * mPagedView.getChildCount());
        ...
        public static class Accordion extends TransitionEffect {
            public Accordion(PagedView pagedView) {
                super(pagedView, TRANSITION_EFFECT_FOLD);
            }
         ...
         * 翻转 2017/6/19
         */
        public static class Rotation extends TransitionEffect {
            public Rotation(PagedView pagedView) {
                super(pagedView, TRANSITION_EFFECT_TURN);
                //deleted by jubingcheng for JumpEffect-fix start on 2017/6/28
//                mFadeInAdjacentScreens = false;
            ...
            public void onScreenScrolled(View v, int i, float scrollProgress) {
                ...
                if (scrollProgress >= -0.5f && scrollProgress <= 0.5f) {
                    v.setTranslationX(width * scrollProgress);
                    if (v.getVisibility() != VISIBLE) {
                        v.setVisibility(VISIBLE);
                    }
                } else {
                    v.setTranslationX(width * -1.0f * mPagedView.getChildCount());
                }
            }
        }
        public static class Fade extends TransitionEffect {

            public Fade(PagedView pagedView) {
                super(pagedView, TRANSITION_EFFECT_FADE);
            }
        ...
        //linhai 创建各个动画子类 end 2017/6/19

    }

    // add for folder icon by liuqiushou 20170620 @{
    protected void setIsSupportCircular(boolean value) {
        mIsSupportCircular = value;
    }

    protected void setIsSupportReboundEffect(boolean value) {
        mHasReboundEffect = value;
    }
    //@}
}
