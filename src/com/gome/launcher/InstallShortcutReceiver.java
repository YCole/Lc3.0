...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    private static void queuePendingShortcutInfo(PendingInstallShortcutInfo info, Context context) {
        ...
        ShortcutInfo item = info.getShortcutInfo();
        if (app.isDisableAllApps() && app.getModel().shortcutExists(context, item)) {
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "shortcutInfo exists, item >" + item);
            }
            return;
        }
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "shortcutInfo don't exists, item>" + item);
        }
