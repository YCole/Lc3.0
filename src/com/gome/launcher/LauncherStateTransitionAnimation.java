...
public class LauncherStateTransitionAnimation {
    ...
    public void startAnimationToWidgets(final Workspace.State fromWorkspaceState,
        ...
        final WidgetsContainerViewGM toView = mLauncher.getWidgetsView();// mod by rongwenzhao from WidgetsContainerView to WidgetsContainerViewGM 2017-6-27
    ...
    private void startAnimationToWorkspaceFromWidgets(final Workspace.State fromWorkspaceState,
            final Workspace.State toWorkspaceState, final int toWorkspacePage,
            final boolean animated, final Runnable onCompleteRunnable) {
        final WidgetsContainerViewGM  widgetsView = mLauncher.getWidgetsView();//// mod by rongwenzhao from WidgetsContainerView to WidgetsContainerViewGM 2017-6-27
    ...
    private AnimatorSet startAnimationToWorkspaceFromOverlay(
        ...
        //add by rongwenzhao 2017-6-27  enter animation begin
        final boolean isFromWidgetToSpringLoaded = fromWorkspaceState == Workspace.State.OVERVIEW && toWorkspaceState == Workspace.State.SPRING_LOADED;
        if (isFromWidgetToSpringLoaded) {
            if (animated && initialized) {
                // Play the workspace animation
                if (workspaceAnim != null) {
                    animation.play(workspaceAnim);
            ...
            //add by rongwenzhao 2017-6-27  enter animation end
        } else {
            if (animated && initialized) {
                // Play the workspace animation
                if (workspaceAnim != null) {
    ...
    private void startWorkspaceSearchBarAnimation(
	    ...
	    //add linhai no space show view back to Workspace start 2017/10/17
        if (SearchDropTargetBar.State.DROP_TARGET != toSearchBarState)
        {
            mLauncher.getSearchDropTargetBar().animateToStateForToastLy(toSearchBarState, duration);
        }
		//end linhai no space show view back to Workspace start 2017/10/17
    }
