...
public class LauncherProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        if (count > 0) notifyListeners(false);
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        Object object = values.get(LauncherSettings.Favorites.STATUS);
        //没有STATUS，表示正常的安装卸载更新，需要刷新。   有STATUS表示是下载中的应用触发的update，不需要刷文件夹+号页
        if (count > 0) notifyListeners(object == null ? false : true);
     ...
     * Overridden in tests
     */
    protected void notifyListeners(boolean isDownloadingUpdate) {
        // always notify the backup agent
        LauncherBackupAgentHelper.dataChanged(getContext());
        if (mListener != null) {
            mListener.onLauncherProviderChange(isDownloadingUpdate);
        }
    }
