...
import com.gome.launcher.accessibility.DragAndDropAccessibilityDelegate;
import com.gome.launcher.accessibility.FolderAccessibilityHelper;
import com.gome.launcher.accessibility.WorkspaceAccessibilityHelper;
import com.gome.launcher.util.DLog;
import com.gome.launcher.util.ParcelableSparseArray;
...
public class CellLayout extends ViewGroup implements BubbleTextView.BubbleTextShadowHandler {
    ...
    private  int mNumHotseatIconsMax;
    @Thunk int mWidthGap;
    ...
    private ViewGroup mPageSetting ;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mNumHotseatIconsMax = grid.inv.numHotseatIconsMax;
        ...
        mPageSetting = (ViewGroup) mLauncher.getLayoutInflater()
                .inflate(R.layout.page_setting, this, false);
        mPageSetting.setVisibility(View.GONE);
        float scale = 100f / res.getInteger(R.integer.config_workspaceOverviewShrinkPercentage);
        mPageSetting.setScaleX(scale);
        mPageSetting.setScaleY(scale);
        ...
        mShortcutsAndWidgets.setTag(Utilities.SHORTCUT_WIDGET_CONTAINER);
     ...
     * @return
     */
    public boolean isTouchPointInView(View view, MotionEvent ev) {
        // update by huanghaihao in 2017-7-27 for screen setting start
        if (view == null || !(view.getVisibility() == VISIBLE && view.getAlpha() > 0)) {
            return false;
        }
        // update by huanghaihao in 2017-7-27 for screen setting end
    ...
    void setIsDragOverlapping(boolean isDragOverlapping) {
        if (mIsDragOverlapping != isDragOverlapping) {
            ...
            //modify by chenchao 2017.8.3  do not use background transition
...
//            invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mDragOutlines.length; i++) {
            ...
            if (alpha > 0) {
				...
				//linhai add 判断坑位是否有显示 容错判断 start 2017/7/18
                if (isHotseat())
                {
                    if (mDragCell[0] >= 0 && mDragCell[1]>=0)
                    {
                        View view = this.getShortcutsAndWidgets().getChildAt(mDragCell[0],mDragCell[1]);
                        if (view != null)
                        {
    ...
    void cellToPoint(int cellX, int cellY, int[] result) {
        final int hStartPadding = getPaddingLeft();
        final int vStartPadding = getPaddingTop();
        //linhai update 计算hotseat动态坑位坐标 start 2017/07/13
        if(LauncherAppState.isDisableAllApps() && mIsHotseat) {
            Log.d("Test","mCountX"+mCountX);
            result[0] = hStartPadding + cellX * (mCellWidth + mWidthGap) + (mNumHotseatIconsMax - mCountX) * (mCellWidth + mWidthGap) / 2 ;
        }else
        {
            result[0] = hStartPadding + cellX * (mCellWidth + mWidthGap);
        }
        //linhai end 2017/07/13
    ...
    void regionToCenterPoint(int cellX, int cellY, int spanX, int spanY, int[] result) {
        ...
        if(LauncherAppState.isDisableAllApps() && mIsHotseat)
        {
            result[0] = hStartPadding + cellX * (mCellWidth + mWidthGap) + (mNumHotseatIconsMax-mCountX)*(mCellWidth + mWidthGap)/2+(spanX * mCellWidth + (spanX - 1) * mWidthGap) / 2;
        }else
        {
            result[0] = hStartPadding + cellX * (mCellWidth + mWidthGap) +
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		//linhai add adaptation count end 2017/08/11
        if (mFixedCellWidth < 0 || mFixedCellHeight < 0) {
	    	//linhai update  start 2017/07/13
            int cw = DeviceProfile.calculateCellWidth(childWidthSize, mIsHotseat ? mNumHotseatIconsMax:mCountX);
			//linhai end 2017/07/13
        ...
        //Added by huanghaihao in 2017-6-26 for screen setting start
        measureChild(mPageSetting, widthMeasureSpec, heightMeasureSpec);
        //Added by huanghaihao in 2017-6-26 for screen setting end
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        mTouchFeedbackView.layout(left, top,
                 mTouchFeedbackView.getMeasuredWidth(),
                 mTouchFeedbackView.getMeasuredHeight());
        mShortcutsAndWidgets.layout(left, top,
                r - l - left,
                b - t - top);
        //Added by huanghaihao in 2017-6-26 for screen setting start
        int settingLeft = (r - l - mPageSetting.getMeasuredWidth()) / 2;
        int settingTop = mPageSetting.getMeasuredHeight() * 7 / 5;
        mPageSetting.layout(settingLeft, -settingTop,
                settingLeft + mPageSetting.getMeasuredWidth(),
                -settingTop + mPageSetting.getMeasuredHeight());
        //Added by huanghaihao in 2017-6-26 for screen setting end
    ...
    public int getUnusedHorizontalSpace() {
	    //linhai update 兼容动态坑位的宽度 start 2017/07/13
        return getMeasuredWidth() - getPaddingLeft() - getPaddingRight() - ((mIsHotseat ? mNumHotseatIconsMax:mCountX) * mCellWidth);
		//linhai end 2017/07/13
    }
    ...
    public void setShortcutAndWidgetAlpha(float alpha) {
        ...
        //Added by huanghaihao in 2017-6-26 for screen setting start
        mPageSetting.setAlpha(alpha);
        //Added by huanghaihao in 2017-6-26 for screen setting end
    }

    //Added by huanghaihao in 2017-6-26 for screen setting
    public ViewGroup getPageSetting() {
        return mPageSetting;
    }
    ...
    void visualizeDropLocation(View v, Bitmap dragOutline, int cellX, int cellY, int spanX,
        ...
        //mod for avoiding npe of dragView by liuqiushou 20170704 @{
        if ((cellX != oldDragCellX || cellY != oldDragCellY) && dragObject.dragView != null) {
             //@}
    ...
    /**
     * zhaosuzhou add for neat icon
     * rongwenzhao copy from gm02 to gm7.0 date:2017-6-20
     * @param useTempCoords
     */
    public void setUseTempCoordsForNeatIcon(boolean useTempCoords) {
        setUseTempCoords(useTempCoords);
    }
    ...
    private ItemConfiguration findConfigurationNoShuffle(int pixelX, int pixelY, int minSpanX, int minSpanY,
        ...
        findNearestVacantArea(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, result,
                resultSpan);
        // modified by liuning for GM12E-434 We think the solution is sucessful only if the target cell and the span are both valid on 2017/12/16
        if (result[0] >= 0 && result[1] >= 0 && resultSpan[0] > 0 && resultSpan[1] > 0) {
            copyCurrentStateToSolution(solution, false);
            solution.dragViewX = result[0];
            solution.dragViewY = result[1];
    ...
    */
     ...
     * Set occupancy
     */
    public void markCellsForNeatIcon(int cellX, int cellY, int spanX, int spanY,boolean value) {
        DLog.e(TAG,"CellLayout --> markCellsForNeatIcon ");
        markCellsForView(cellX,cellY,spanX,spanY,mOccupied,value);
    }
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public void setup(int cellWidth, int cellHeight, int widthGap, int heightGap,
                          boolean invertHorizontally, int colCount,boolean mIsHotseatLayout,int mNumHotseatIconsMax) {
            if (isLockedToGrid) {
                ...
                //linhai update 计算hotseat动态坑位坐标 start 2017/07/13
                if (LauncherAppState.isDisableAllApps() &&mIsHotseatLayout)
                {

                    x = (int) (myCellX * (cellWidth + widthGap ) + (mNumHotseatIconsMax-colCount)*(cellWidth + widthGap)/2)+leftMargin;
                }else
                {
