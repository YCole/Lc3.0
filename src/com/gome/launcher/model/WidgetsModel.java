...
package com.gome.launcher.model;
...
import com.gome.launcher.AppFilter;
import com.gome.launcher.IconCache;
import com.gome.launcher.InvariantDeviceProfile;
...
import com.gome.launcher.compat.AlphabeticIndexCompat;
...
public class WidgetsModel {
    ...
    private List<String> mDefaultWidgetsList;
    ...
    public WidgetsModel(Context context,  IconCache iconCache, AppFilter appFilter) {
        ...
        //added by liuning for sorting widgets on 2017/11/2 start
        String[] defaultWidgetArrays = context.getResources().getStringArray(R.array.default_widget_list);
        mDefaultWidgetsList = Arrays.asList(defaultWidgetArrays);
        //added by liuning for sorting widgets on 2017/11/2 end
    ...
    /**
     * added by liuning for sorting widget on 2017/11/2
     * 排序规则：default_widget_list配置 > 系统应用（内置不可卸载+更新） > 内置可卸载 + 安装
     */
    public void sortWidgets() {
        ArrayList<PackageItemInfo> tmpPackageItemInfos = new ArrayList<>();
        Iterator<PackageItemInfo> ite;
        for (String s : mDefaultWidgetsList) {
            ite = mPackageItemInfos.iterator();
            while (ite.hasNext()) {
                PackageItemInfo p = ite.next();
                if (s.equals(p.packageName.toString())) {
                    tmpPackageItemInfos.add(p);
                    ite.remove();
        ...
        while (ite.hasNext()) {
            PackageItemInfo p = ite.next();
            if (Utilities.isSystemApp(mContext, p.packageName)) {
                tmpPackageItemInfos.add(p);
                ite.remove();
            }
        }
        tmpPackageItemInfos.addAll(mPackageItemInfos);
        mPackageItemInfos.clear();
        mPackageItemInfos.addAll(tmpPackageItemInfos);
