...
public class PageIndicator extends LinearLayout {
    ...
    public PageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        mSingleShow = a.getBoolean(R.styleable.PageIndicator_singleShow, true);
        mWindowRange[0] = 0;
        mWindowRange[1] = 0;
     ...
     * @param singleShow
     */
    public void setVisibility(boolean singleShow) {
    ...
    void updateMarker(int index, PageMarkerResources marker) {
        // added by jubingcheng for fix[PRODUCTION-11687] on 2017/12/20 start
        if (mMarkers.size() > 0) {
//            index = Math.max(0, Math.min(mMarkers.size() - 1, index));
            //为了抓取出错信息，改为条件判断
            if(index < 0)
            {
                Log.e(TAG, "updateMarker index:" + index + " size:" + mMarkers.size());
                index = 0;
            } else if (index > mMarkers.size() - 1)
            {
                Log.e(TAG, "updateMarker index:" + index + " size:" + mMarkers.size());
                index = mMarkers.size() - 1;
            }
            // added by jubingcheng for fix[PRODUCTION-11687] on 2017/12/20 end
