...
import com.gome.launcher.NycWallpaperUtils;
...
public class BitmapCropTask extends AsyncTask<Integer, Void, Boolean> {
            ...
            if (crop.compress(CompressFormat.JPEG, DEFAULT_COMPRESS_QUALITY, tmpOut)) {
                ...
                if (mSetWallpaper) {
                    try {
                        ...
                        // Added by wjq in 2017-09-28 for // fix bug for 9250 start
                        if (whichWallpaper == WallpaperManager.FLAG_LOCK ||
                                whichWallpaper == (WallpaperManager.FLAG_SYSTEM | WallpaperManager.FLAG_LOCK)) {
                        // Added by wjq in 2017-09-28 for // fix bug for 9250 end
                            Intent intent = new Intent("com.gome.launcher.LOCK_SCREEN_DATA");
                            intent.putExtra("LOCK_SCREEN",outByteArray);
                            mContext.sendBroadcast(intent);
                            Log.e("plory","outByteArray = " + outByteArray.length);
                        }
    ...
    protected void onPostExecute(Boolean cropSucceeded) {
        if (!cropSucceeded) {
            Toast.makeText(mContext, com.gome.launcher.R.string.wallpaper_set_fail, Toast.LENGTH_SHORT).show();
        }
